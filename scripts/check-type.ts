...
const getCommandRetVal = (command: string) => {
  return execSync(command).toString('utf8').trim()
}

// 根据文件路径向上查找 tsconfig.json
const getTsConfigPath = (filePath: string) => {
...
const tsconfigMap = filesToCheck.reduce((acc: Record<string, string[]>, file: string) => {
...
Object.keys(tsconfigMap)
  .filter((file) => !!file && !file.endsWith('creator-fe/tsconfig.json'))
  .forEach((tsconfigFile) => {
    ...
    exec(`tsc -p ${tsconfigLintPath} --noEmit`, (err: Error, stdout: string) => {
